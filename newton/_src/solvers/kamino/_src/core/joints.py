...
class JointDoFType(IntEnum):
    ...
    **DISCLAIMER**: This joint is not yet fully supported, and currently behaves
    ...
    def axes_matrix_from_joint_type(
        ...
        if dof_type == wp.static(JointDoFType.FIXED):
            pass  # R_axis_j is already set to identity
        elif dof_type == wp.static(JointDoFType.REVOLUTE):
            R_axis_j = _axis_rotmatn_from_vec3f(dof_axes[0])
        elif dof_type == wp.static(JointDoFType.PRISMATIC):
            R_axis_j = _axis_rotmatn_from_vec3f(dof_axes[0])
        elif dof_type == wp.static(JointDoFType.CYLINDRICAL):
            R_axis_j = _axis_rotmatn_from_vec3f(dof_axes[0])
        elif dof_type == wp.static(JointDoFType.UNIVERSAL):
            ax = dof_axes[0]
            ay = dof_axes[1]
            az = wp.cross(ax, ay)
            R_axis_j = wp.matrix_from_cols(ax, ay, az)
        elif dof_type == wp.static(JointDoFType.SPHERICAL):
            R_axis_j = wp.matrix_from_cols(dof_axes[0], dof_axes[1], dof_axes[2])
        elif dof_type == wp.static(JointDoFType.CARTESIAN):
            R_axis_j = wp.matrix_from_cols(dof_axes[0], dof_axes[1], dof_axes[2])

        elif dof_type == wp.static(JointDoFType.FIXED):
            ...
            R_axis_j = wp.matrix_from_cols(dof_axes[0], dof_axes[1], dof_axes[2])
        elif dof_type == wp.static(JointDoFType.FREE):
            assert wp.norm_l2(dof_axes[0] - dof_axes[3]) < 1e-6, "Linear and rotational axes for fixed joint must match"
            assert wp.norm_l2(dof_axes[1] - dof_axes[4]) < 1e-6, "Linear and rotational axes for fixed joint must match"
            assert wp.norm_l2(dof_axes[2] - dof_axes[5]) < 1e-6, "Linear and rotational axes for fixed joint must match"
            R_axis_j = wp.matrix_from_cols(dof_axes[0], dof_axes[1], dof_axes[2])
...
def joint_dof_type_from_newton(
    ...
    """
    # First try directly mapping the trivially supported types
    if joint_type == wp.static(JointType.PRISMATIC):
        return wp.static(JointDoFType.PRISMATIC)
    elif joint_type == wp.static(JointType.REVOLUTE):
        return wp.static(JointDoFType.REVOLUTE)
    elif joint_type == wp.static(JointType.BALL):
        return wp.static(JointDoFType.SPHERICAL)
    elif joint_type == wp.static(JointType.FIXED):
        return wp.static(JointDoFType.FIXED)
    elif joint_type == wp.static(JointType.FREE):
        return wp.static(JointDoFType.FREE)
    ...
    if q_count == 0 and qd_count == 0 and dof_dim == vec2i(0, 0):
        ...
        return wp.static(JointDoFType.UNIVERSAL)
    elif q_count == 2 and qd_count == 2 and dof_dim == vec2i(1, 1):
        return wp.static(JointDoFType.CYLINDRICAL)
    elif q_count == 3 and qd_count == 3 and dof_dim == vec2i(3, 0):
        return wp.static(JointDoFType.CARTESIAN)
...
def joint_actuation_mode_from_newton(target_mode: int) -> int:
    ...
    """
    if target_mode == wp.static(JointTargetMode.NONE):
        return wp.static(JointActuationType.PASSIVE)
    if target_mode == wp.static(JointTargetMode.EFFORT):
        return wp.static(JointActuationType.FORCE)
    if target_mode == wp.static(JointTargetMode.POSITION):
        return wp.static(JointActuationType.POSITION)
    if target_mode == wp.static(JointTargetMode.VELOCITY):
        return wp.static(JointActuationType.VELOCITY)
    if target_mode == wp.static(JointTargetMode.POSITION_VELOCITY):
        return wp.static(JointActuationType.POSITION_VELOCITY)
...
def num_coords_from_dof_type(dof_type: int) -> int:
    ...
    Returns:
        ...
        invalid.
    """
    if dof_type == wp.static(JointDoFType.FREE):
        return 7  # 3D position + 4D quaternion
    elif dof_type == wp.static(JointDoFType.REVOLUTE):
        return 1  # 1D angle
    elif dof_type == wp.static(JointDoFType.PRISMATIC):
        ...
        return 4  # 4D unit-quaternion
    elif dof_type == wp.static(JointDoFType.GIMBAL):
        return 3  # 3D euler angles
    elif dof_type == wp.static(JointDoFType.CARTESIAN):
        return 3  # 3D distances
    elif dof_type == wp.static(JointDoFType.FIXED):
