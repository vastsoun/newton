...
class DelassusOperatorSparse(BlockSparseLinearOperators):
    ...
    def finalize(
        ...
        # Ensure the model container is valid
        if model is None:
            raise ValueError("A model container of type `ModelKamino` must be provided to allocate the Delassus operator.")
        elif not isinstance(model, ModelKamino):
            raise ValueError("Invalid model provided. Must be an instance of `ModelKamino`.")

        # Ensure the data container is valid if provided
        if data is None:
            raise ValueError("A data container of type `DataKamino` must be provided to allocate the Delassus operator.")
        elif not isinstance(data, DataKamino):
            raise ValueError("Invalid data container provided. Must be an instance of `DataKamino`.")
    ...
    def _apply_inverse_mass_matrix(self, x: wp.array, world_mask: wp.array):
        ...
        """
        if self._model is None or self._data is None:
            raise RuntimeError("ModelKamino and data must be assigned before applying inverse mass matrix.")
    ...
    def _apply_regularization(self, x: wp.array, y: wp.array, world_mask: wp.array, alpha: float = 1.0):
        ...
        """
        if self._model is None or self._data is None:
            raise RuntimeError("ModelKamino and data must be assigned before applying regularization.")
    ...
    def _apply_preconditioning(self, x: wp.array, world_mask: wp.array):
        ...
        """
        if self._model is None or self._data is None:
            raise RuntimeError("ModelKamino and data must be assigned before applying preconditioning.")
