...
def _compute_joint_parent_wrenches_dense(
    ...
    w_F_j_dof_act = vec6f(0.0)
    w_F_j_cts_dyn = vec6f(0.0)
    w_F_j_cts_kin = vec6f(0.0)
...
def _compute_joint_parent_wrenches_sparse(
    ...
    data_joints_tau_j: wp.array[float32],
    jac_dofs_joint_nzb_offsets: wp.array[int32],
    jac_dofs_nzb_values: wp.array[vec6f],
    jac_cts_joint_nzb_offsets: wp.array[int32],
...
def _add_limit_to_joint_parent_wrench_sparse(
    ...
    limits_jid: wp.array[int32],
    limits_lid: wp.array[int32],
    jac_cts_limit_nzb_offsets: wp.array[int32],
    jac_cts_nzb_values: wp.array[vec6f],
    lambdas_offsets: wp.array[int32],
    lambdas_data: wp.array[float32],
    # Outputs:
...
def compute_joint_dof_body_wrenches_dense(
    ...
    wp.launch(
        kernel=_compute_joint_dof_body_wrenches_dense,
...
def compute_joint_dof_body_wrenches_sparse(
    ...
    # Then compute the body wrenches resulting from the current generalized actuation forces
    wp.launch(
        kernel=_compute_joint_dof_body_wrenches_sparse,
        dim=model.size.sum_of_num_joints,
        inputs=[
...
def compute_constraint_body_wrenches_dense(
    ...
    if model.size.sum_of_num_joints > 0:
        ...
        wp.launch(
            kernel=_compute_joint_cts_body_wrenches_dense,
            dim=model.size.sum_of_num_joints,
            inputs=[
                model.info.bodies_offset,
    ...
    if _has_limits(limits):
        ...
        wp.launch(
            kernel=_compute_limit_cts_body_wrenches_dense,
    ...
    if _has_contacts(contacts):
        ...
        wp.launch(
            kernel=_compute_contact_cts_body_wrenches_dense,
...
def compute_constraint_body_wrenches_sparse(
    ...
    wp.launch(
        kernel=_compute_cts_body_wrenches_sparse,
