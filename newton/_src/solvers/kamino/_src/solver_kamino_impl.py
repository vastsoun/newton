...
class SolverKaminoImpl(SolverBase):
    ...
    def step(
        ...
        # ncts = self.problem_fd.data.dim.numpy()[0]
        # status = self.solver_fd.data.status.numpy()[0]
        # msg.error("padmm.converged: %s", status[0])
        # msg.error("padmm.iterations: %s", status[1])
        # msg.error("padmm.r_p: %s", status[2])
        # msg.error("padmm.r_d: %s", status[3])
        # msg.error("padmm.r_c: %s", status[4])
        # msg.error("padmm.r_dx: %s", status[5])
        # msg.error("padmm.r_dy: %s", status[6])
        # msg.error("padmm.r_dz: %s\n", status[7])

        # msg.error("ncts: %s", ncts)
        # msg.error("lambdas: %s", self.solver_fd.data.solution.lambdas.numpy()[:ncts])
        # msg.error("v_plus: %s\n", self.solver_fd.data.solution.v_plus.numpy()[:ncts])

        # msg.warning("state_in.q_i:\n%s", state_in.q_i)
        # msg.warning("state_in.u_i:\n%s", state_in.u_i)
        # msg.warning("state_in.w_i:\n%s\n", state_in.w_i)
        # msg.notif("state_out.q_i:\n%s", state_out.q_i)
        # msg.notif("state_out.u_i:\n%s", state_out.u_i)
        # msg.notif("state_out.w_i:\n%s\n", state_out.w_i)
    ...
    def _update_dynamics(self, contacts: ContactsKamino | None = None):
        ...
        # nc = self._problem_fd.data.nc.numpy()[0]
        # ncts = self._problem_fd.data.dim.numpy()[0]
        # msg.warning("problem_fd.data.nc: %s", nc)
        # msg.warning("problem_fd.data.ncts: %s", ncts)
        # msg.warning("problem_fd.data.mu: %s", self._problem_fd.data.mu[:nc])
        # msg.warning("problem_fd.data.v_b: %s", self._problem_fd.data.v_b[:ncts])
        # msg.warning("problem_fd.data.v_i: %s", self._problem_fd.data.v_i[:ncts])
        # msg.warning("problem_fd.data.v_f: %s\n", self._problem_fd.data.v_f[:ncts])
    ...
    def _update_constraints(self, contacts: ContactsKamino | None = None):
        ...
        # msg.notif("contacts.reaction:\n%s", contacts.reaction.numpy()[:nc])
        # msg.notif("contacts.velocity:\n%s\n", contacts.velocity.numpy()[:nc])
