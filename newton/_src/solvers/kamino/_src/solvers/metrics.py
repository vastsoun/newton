...
def _compute_joint_kinematics_residual(
    ...
    if r_kinematics_j >= previous_max:
        argmax_key = wp.int64(build_pair_key2(wp.uint32(jid), wp.uint32(local_cts_offset_j) + kin_argmax_local))
...
class SolutionMetrics:
    ...
    def finalize(self, model: ModelKamino, data: DataKamino):
        ...
        # Store a reference to the model and data for internal use
        self._model = model
        self._data = data
        ...
        with wp.ScopedDevice(self._device):
            ...
            self._buffer_v = wp.zeros(self._model.size.sum_of_max_total_cts, dtype=wp.float32)
            self._buffer_s = wp.zeros(self._model.size.sum_of_max_total_cts, dtype=wp.float32)
            ...
            self._metrics = SolutionMetricsData(
                r_eom=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_eom_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int64),
                r_kinematics=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_kinematics_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int64),
                r_cts_joints=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_cts_joints_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int64),
                r_cts_limits=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_cts_limits_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int64),
                r_cts_contacts=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_cts_contacts_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int32),
                r_v_plus=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_v_plus_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int32),
                r_ncp_primal=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_ncp_primal_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int32),
                r_ncp_dual=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_ncp_dual_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int32),
                r_ncp_compl=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_ncp_compl_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int32),
                r_vi_natmap=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                r_vi_natmap_argmax=wp.full(self._model.size.num_worlds, value=-1, dtype=wp.int32),
                f_ncp=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
                f_ccp=wp.zeros(self._model.size.num_worlds, dtype=wp.float32),
            )
