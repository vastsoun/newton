...
class KKTSystemState:
    ...
    def __init__(self):
        ...
        """
        The max dimensions of the symmetric KKT matrix of each world.\n
        ...
        self.K: wp.array(dtype=float32) | None = None
        """
        The flat KKT system matrix.\n
        Shape of ``(sum(maxdim_w * maxdim_w),)`` and type :class:`float32`.\n
        `maxdim_w` is the maximum dimensions of the KKT matrix block of each world.
        """

        self.k: wp.array(dtype=float32) | None = None
        """
        The flat KKT system vector.\n
        Shape of ``(sum(maxdim_w * maxdim_w),)`` and type :class:`float32`.\n
        `maxdim_w` is the maximum dimensions of the KKT matrix block of each world.
        """
...
class KKTSystem:
    ...
    def __init__(
        ...
        factorizer: LDLTFactorizer = None,
        ...
        Args:
            ...
            factorizer (LDLTFactorizer, optional): An optional LDLT factorization object. Defaults to None.
        """
        ...
        # Initialize the KKT system state container
        self._data: KKTSystemState = KKTSystemState()

        # Declare the optional LDLT factorization
        self._factorizer: LDLTFactorizer = None
    ...
    @property
    def factorizer(self) -> LDLTFactorizer:
        """
        The LDLT factorization object for the Delassus operator.
    ...
    def allocate(
        ...
        self._data.K = wp.zeros(shape=(self._model_maxsize,), dtype=float32, device=self._device)
    ...
    def build(
        ...
        # Build the KKT matrix parrallelized element-wise
        wp.launch(
            kernel=_build_kkt_matrix_elementwise,
    ...
    def regularize(self, eta: wp.array):
        """
        Adds diagonal regularization to each matrix block of the KKT matrix.
    ...
    def factorize(self, reset_to_zero: bool = True):
        """
        Factorizes the Delassus matrix using the LDLT factorization.\n
        ...
        if self._factorizer is None:
            raise ValueError("LDLT factorizer is not available. Allocate with factorizer=LDLTFactorizer.")
    ...
    def solve(self, v: wp.array, x: wp.array):
        """
        Solves the linear system D * x = v using the LDLT factorization.
