...
from ..linalg.sparse_matrix import BlockDType, BlockSparseMatrices, FixedRowBlockSparseMatrices
...
def make_store_joint_jacobian_sparse_fixed_row_func(axes: Any):
    """
    Generates a warp function to store body-pair Jacobian blocks into a target
    block sparse data structure.
    """
    ...
    @wp.func
    def store_joint_jacobian_sparse_fixed_row(
        wid: int,
        row_start: int,
        bid_offset: int,
        bid_B: int,
        ...
        # Retrieve the start index of the joint's non-zero blocks
        nzb_start = J_nzb_start[wid] + row_start * 2

        # Store the Jacobian block for the follower body
        J_F_col = 6 * (bid_F - bid_offset)
        for i in range(num_jac_rows):
            J_F_i_nzb_start = nzb_start + 2 * i
            J_nzb_coords[J_F_i_nzb_start, 0] = row_start + i
            J_nzb_coords[J_F_i_nzb_start, 1] = J_F_col
            J_nzb_values[J_F_i_nzb_start] = JT_F_j[:, axes[i]]
        ...
        if bid_B > -1:
            J_B_col = 6 * (bid_B - bid_offset)
            for i in range(num_jac_rows):
                J_B_i_nzb_start = nzb_start + 2 * i + 1
                J_nzb_coords[J_B_i_nzb_start, 0] = row_start + i
                J_nzb_coords[J_B_i_nzb_start, 1] = J_B_col
                J_nzb_values[J_B_i_nzb_start] = JT_B_j[:, axes[i]]

    # Return the function
    return store_joint_jacobian_sparse_fixed_row
...
@wp.func
def store_joint_cts_jacobian_sparse_fixed_row(
    wid: int,
    dof_type: int,
    cts_offset: int,
    bid_offset: int,
    ...
    if dof_type == JointDoFType.REVOLUTE:
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.REVOLUTE.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.PRISMATIC.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.CYLINDRICAL.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.UNIVERSAL.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.SPHERICAL.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.GIMBAL.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.CARTESIAN.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.FIXED.cts_axes))(
            wid,
            cts_offset,
            bid_offset,
            bid_B,
...
@wp.func
def store_joint_dofs_jacobian_sparse_fixed_row(
    wid: int,
    dof_type: int,
    dofs_offset: int,
    bid_offset: int,
    ...
    if dof_type == JointDoFType.REVOLUTE:
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.REVOLUTE.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.PRISMATIC.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.CYLINDRICAL.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.UNIVERSAL.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.SPHERICAL.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.GIMBAL.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.CARTESIAN.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
        ...
        wp.static(make_store_joint_jacobian_sparse_fixed_row_func(JointDoFType.FREE.dofs_axes))(
            wid,
            dofs_offset,
            bid_offset,
            bid_B,
...
@wp.kernel
def _configure_jacobians_sparse_fixed_row(
    # Input:
    model_num_body_dofs: wp.array(dtype=int32),
    model_num_joint_cts: wp.array(dtype=int32),
    model_num_joint_dofs: wp.array(dtype=int32),
    ...
    # Output:
    jac_cts_dims: wp.array2d(dtype=int32),
    jac_cts_num_nzb: wp.array(dtype=int32),
    jac_dofs_dims: wp.array2d(dtype=int32),
    jac_dofs_num_nzb: wp.array(dtype=int32),
):
    world_id = wp.tid()

    jac_cts_dims[world_id, 0] = model_num_joint_cts[world_id] + num_limits[world_id] + 3 * num_contacts[world_id]
    jac_cts_dims[world_id, 1] = model_num_body_dofs[world_id]
    jac_cts_num_nzb[world_id] = 2 * jac_cts_dims[world_id, 0]

    jac_dofs_dims[world_id, 0] = model_num_joint_dofs[world_id]
    jac_dofs_dims[world_id, 1] = model_num_body_dofs[world_id]
    jac_dofs_num_nzb[world_id] = 2 * model_num_joint_dofs[world_id]
...
@wp.kernel
def _build_joint_jacobians_sparse_fixed_row(
    # Inputs
    model_info_bodies_offset: wp.array(dtype=int32),
    model_joints_wid: wp.array(dtype=int32),
    model_joints_dof_type: wp.array(dtype=int32),
    ...
    # Store the constraint Jacobian block
    store_joint_cts_jacobian_sparse_fixed_row(
        wid,
        dof_type,
        cio,
        bio,
    ...
    # Store the actuation Jacobian block if the joint is actuated
    store_joint_dofs_jacobian_sparse_fixed_row(
        wid,
        dof_type,
        dio,
        bio,
...
@wp.kernel
def _build_limit_jacobians_sparse_fixed_row(
    # Inputs:
    model_info_bodies_offset: wp.array(dtype=int32),
    state_info_limit_cts_group_offset: wp.array(dtype=int32),
    limits_model_num: wp.array(dtype=int32),
    ...
    # Compute the index of the blocks we need to pull from the dofs Jacobian to reuse for the limits.
    jac_dofs_nzb_idx = jacobian_dofs_nzb_start[world_id] + dof_l * 2
    ...
    # Set the constraint Jacobian block for the follower body from the actuation Jacobian block
    jac_cts_nzb_offset_world = 2 * (limit_cts_offset + limit_id_l)
    jac_cts_nzb_idx = jacobian_cts_nzb_start[world_id] + jac_cts_nzb_offset_world
    jacobian_cts_nzb_values[jac_cts_nzb_idx] = side_l * jacobian_dofs_nzb_values[jac_dofs_nzb_idx]
    jacobian_cts_nzb_coords[jac_cts_nzb_idx, 0] = limit_cts_offset + limit_id_l
    jacobian_cts_nzb_coords[jac_cts_nzb_idx, 1] = 6 * (body_id_F_l - body_index_offset)
...
@wp.kernel
def _build_contact_jacobians_sparse_fixed_row(
    # Inputs:
    model_info_bodies_offset: wp.array(dtype=int32),
    state_info_contact_cts_group_offset: wp.array(dtype=int32),
    state_bodies_q: wp.array(dtype=transformf),
    ...
    body_idx_offset_B = 6 * (body_id_B_k - body_idx_offset)
    # Allocate non-zero blocks in the Jacobian by incrementing the number of NZB
    jac_cts_nzb_offset_world = 2 * cts_idx_start
    jac_cts_nzb_offset = jacobian_cts_nzb_start[world_id] + jac_cts_nzb_offset_world
    # Store 3x6 Jacobian block as three separate 1x6 blocks
    for j in range(3):
        jacobian_cts_nzb_values[jac_cts_nzb_offset + 2 * j] = JT_c_B_k[:, j]
        jacobian_cts_nzb_coords[jac_cts_nzb_offset + 2 * j, 0] = cts_idx_start + j
        jacobian_cts_nzb_coords[jac_cts_nzb_offset + 2 * j, 1] = body_idx_offset_B
    ...
    if body_id_A_k > -1:
        ...
        # Store 3x6 Jacobian block as three separate 1x6 blocks
        for j in range(3):
            jacobian_cts_nzb_values[jac_cts_nzb_offset + 2 * j + 1] = JT_c_A_k[:, j]
            jacobian_cts_nzb_coords[jac_cts_nzb_offset + 2 * j + 1, 0] = cts_idx_start + j
            jacobian_cts_nzb_coords[jac_cts_nzb_offset + 2 * j + 1, 1] = body_idx_offset_A
...
class SparseSystemJacobians:
    ...
    def __init__(
        ...
        if model is not None:
            self.finalize(
                model=model, limits=limits, contacts=contacts, device=device, fixed_row_matrices=fixed_row_matrices
            )
    ...
    def finalize(
        ...
        if fixed_row_matrices:
            # Add non-zero blocks for joints
            for _j in range(model.size.sum_of_num_joints):
                w = joint_wid[_j]
                J_dofs_joint_nzb_offsets[_j] = J_dofs_nnzb[w]
                J_dofs_nnzb[w] += 2 * int(joint_num_dofs[_j])
            ...
            for _j in range(model.size.sum_of_num_joints):
                ...
                if max_num_limits[w] > 0:
                    for d_j in range(joint_num_dofs[_j]):
                        if joint_q_j_min[dofs_start + d_j] > float(FLOAT32_MIN) or joint_q_j_max[
                            dofs_start + d_j
                        ] < float(FLOAT32_MAX):
    ...
    def build(
        ...
        if isinstance(jacobian_cts, FixedRowBlockSparseMatrices) and isinstance(
            jacobian_dofs, FixedRowBlockSparseMatrices
        ):
            ...
            wp.launch(
                _configure_jacobians_sparse_fixed_row,
            ...
            if model.size.sum_of_num_joints > 0:
                wp.launch(
                    _build_joint_jacobians_sparse_fixed_row,
                    dim=model.size.sum_of_num_joints,
                    inputs=[
                        # Inputs:
                        model.info.bodies_offset,
            ...
            if limits is not None and limits.model_max_limits_host > 0:
                wp.launch(
                    _build_limit_jacobians_sparse_fixed_row,
                    dim=limits.model_max_limits_host,
                    inputs=[
                        # Inputs:
                        model.info.bodies_offset,
            ...
            if contacts is not None and contacts.model_max_contacts_host > 0:
                wp.launch(
                    _build_contact_jacobians_sparse_fixed_row,
                    dim=contacts.model_max_contacts_host,
                    inputs=[
                        # Inputs:
                        model.info.bodies_offset,
