...
@dataclass
class DenseRectangularMultiLinearInfo(LinearInfo):
    """
...
@dataclass
class DenseSquareMultiLinearInfo(LinearInfo):
    """
...
class DenseLinearOperator(LinearOperator):
    ...
    def gemv(self, x: wp.array, y: wp.array, matrix_mask: wp.array, alpha: float = 1.0, beta: float = 0.0) -> None:
        ...
        if self.mat.ndim == 1:
            ...
            blas.dense_gemv(self.mat, x, y, self.info.dim, matrix_mask, alpha, beta, self.info.max_dimension)

    def is_operator_2d(self) -> bool:
        return self.mat.ndim == 2

    def operator_2d(self) -> Self:
