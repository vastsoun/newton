...
def assert_model_matches_builder(test: unittest.TestCase, builder: ModelBuilder, model: Model):
    ...
    for i in range(model.size.sum_of_num_geoms):
        ...
        test.assertEqual(
            ...
            builder.geoms[i].bid + bid_offset if builder.geoms[i].bid >= 0 else -1,
        )
...
class TestModelBuilder(unittest.TestCase):
    ...
    def test_09_add_geometry(self):
        ...
        self.assertEqual(cgid, builder.geoms[cgid].gid)
        self.assertEqual(builder.geoms[cgid].name, "test_geom")
        self.assertEqual(builder.geoms[cgid].layer, "foo")
        self.assertEqual(builder.geoms[cgid].bid, -1)
        self.assertEqual(builder.geoms[cgid].wid, wid)
        self.assertEqual(builder.geoms[cgid].lid, 0)
        self.assertEqual(builder.geoms[cgid].mid, 0)
        self.assertEqual(len(builder.worlds[wid].geometry_layers), 1)
    ...
    def test_10_add_geometry_descriptors(self):
        ...
        self.assertEqual(builder.geoms[gid].layer, "default")
        self.assertEqual(builder.geoms[gid].bid, -1)
        self.assertEqual(builder.geoms[gid].wid, wid)
        self.assertEqual(builder.geoms[gid].lid, 0)
