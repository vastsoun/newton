...
class TestDelassusOperatorSparse(unittest.TestCase):
    ...
    def _check_sparse_delassus_allocations(
        ...
        self.assertEqual(delassus._vec_temp.shape, (model.size.sum_of_num_body_dofs,))

        # # Compute expected and allocated dimensions and sizes
        # expected_max_constraint_dims = get_max_constraints_per_world(model, limits, contacts)
        # num_worlds = len(expected_max_constraint_dims)
        # expected_D_sizes = [
        #     expected_max_constraint_dims[i] * expected_max_constraint_dims[i] for i in range(num_worlds)
        # ]
        # delassus_maxdim_np = delassus.info.maxdim.numpy()
        # self.assertEqual(
        #     len(delassus_maxdim_np),
        #     num_worlds,
        #     "Number of Delassus operator blocks does not match the number of worlds",
        # )
        # D_maxdims = [int(delassus_maxdim_np[i]) for i in range(num_worlds)]
        # D_sizes = [D_maxdims[i] * D_maxdims[i] for i in range(num_worlds)]
        # D_sizes_sum = sum(D_sizes)
        ...
        # # Check Delassus operator data sizes
        # self.assertEqual(delassus.info.maxdim.size, num_worlds)
        # self.assertEqual(delassus.info.dim.size, num_worlds)
        # self.assertEqual(delassus.info.mio.size, num_worlds)
        # self.assertEqual(delassus.info.vio.size, num_worlds)
        # self.assertEqual(delassus.D.size, D_sizes_sum)

        # # Check if the factorizer info data to the same as the Delassus info data
        # self.assertEqual(delassus.info.maxdim.ptr, delassus.solver.operator.info.maxdim.ptr)
        # self.assertEqual(delassus.info.dim.ptr, delassus.solver.operator.info.dim.ptr)
        # self.assertEqual(delassus.info.mio.ptr, delassus.solver.operator.info.mio.ptr)
        # self.assertEqual(delassus.info.vio.ptr, delassus.solver.operator.info.vio.ptr)
    ...
    def test_01_allocate_single_delassus_operator(self):
        ...
        # Compare expected to allocated dimensions and sizes
        self._check_sparse_delassus_allocations(model, limits, detector.contacts, delassus)
    ...
    def test_04_build_delassus_operator(self):
        ...
        for w in range(model.info.num_worlds):
            ...
            if not is_D_close or self.verbose:
                print(f"[{w}]: D_w (shape={D_w.shape}):\n{D_w}")
                print(f"[{w}]: D_np (shape={D_np[w].shape}):\n{D_np[w]}")
                print_error_stats(f"D[{w}]", D_np[w], D_w, D_w.shape[0], show_errors=True)
            self.assertTrue(is_D_close)
