...
class TestUtilsLinAlgLinearSolver(unittest.TestCase):
    ...
    def test_05_defaulted_direct_solver(self):
        ...
        msg.debug(f"solver._factorization_error_abs: {solver._factorization_error_abs}")
        msg.debug(f"solver._factorization_error_rel: {solver._factorization_error_rel}\n")
        eps64 = np.finfo(np.float64).eps
        self.assertEqual(solver._dtype, np.float64)
        self.assertEqual(solver._atol, eps64)
        self.assertEqual(solver._rtol, eps64)
        ...
        self.assertIsNone(solver._factorization_error_abs)
        self.assertIsNone(solver._factorization_error_rel)
        ...
        self.assertIsNotNone(solver._factorization_error_abs)
        self.assertIsNotNone(solver._factorization_error_rel)
        self.assertAlmostEqual(solver._factorization_error_abs, 0.0, places=6)
        self.assertAlmostEqual(solver._factorization_error_rel, 0.0, places=6)
        ...
        x_0 = solver.solve(self.b, compute_error=False)
        msg.debug(f"\nx_0 {x_0.shape}[{x_0.dtype}]:\n{x_0}\n")
        msg.debug(f"solver._error_abs: {solver._error_abs}")
        msg.debug(f"solver._error_rel: {solver._error_rel}\n")
        self.assertTrue(np.allclose(x_0, x_np, atol=self.atol, rtol=self.rtol))
        self.assertIsNone(solver._error_abs)
        self.assertIsNone(solver._error_rel)
        ...
        msg.debug(f"solver._error_rel: {solver._error_rel}\n")
        self.assertTrue(np.allclose(x_1, x_np, atol=self.atol, rtol=self.rtol))
        self.assertIsNotNone(solver._error_abs)
        self.assertIsNotNone(solver._error_rel)
        self.assertAlmostEqual(solver._error_abs, 0.0, places=6)
        self.assertAlmostEqual(solver._error_rel, 0.0, places=6)
    ...
    def test_iterative_solver_methods(self):
        # Print test configuration
        msg.debug("\n----------------------------")
