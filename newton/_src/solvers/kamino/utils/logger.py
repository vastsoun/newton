...
def _copy_padmm_status_row(
    ...
    decision: wp.array[wp.int32],
    log_iterations: wp.array2d[wp.int32],
    log_converged: wp.array2d[wp.int32],
    log_num_restarts: wp.array2d[wp.int32],
    log_r_p: wp.array2d[wp.float32],
    log_r_d: wp.array2d[wp.float32],
    log_r_c: wp.array2d[wp.float32],
):
...
def _copy_padmm_penalty_row(
    ...
    decision: wp.array[wp.int32],
    log_rho: wp.array2d[wp.float32],
    log_num_rho_updates: wp.array2d[wp.int32],
):
...
def _copy_int32_row(
    ...
    """Copy one ``(num_worlds,)`` wp.int32 source row into ``dest[write_idx, :]``.

    Args:
        src: The source per-world wp.int32 array.
        decision: Two-element decision buffer ``[should_write, write_idx]``.
...
def _copy_padmm_iterate_residuals_row(
    src: wp.array[PADMMStatus],
    decision: wp.array[wp.int32],
    log_r_dx: wp.array2d[wp.float32],
    log_r_dy: wp.array2d[wp.float32],
    log_r_dz: wp.array2d[wp.float32],
):
...
def _copy_padmm_acceleration_row(
    ...
    src_a: wp.array[wp.float32],
    src_a_factor: wp.array[wp.float32],
    decision: wp.array[wp.int32],
    log_r_a: wp.array2d[wp.float32],
    log_a: wp.array2d[wp.float32],
    log_a_factor: wp.array2d[wp.float32],
):
    ...
    def __init__(
        ...
        with wp.ScopedDevice(self._device):
            ...
            for field in self._scalar_fields_int32:
                setattr(self, f"log_{field}", wp.zeros(shape=shape, dtype=wp.int32))
