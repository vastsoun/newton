...
class SimulatorData:
    ...
    Attributes:
        state_p (State): holds the 'previous' state data
        state_n (State): holds the 'current' state data, computed from the previous step as:
            ``state_n = f(state_p, control)``, where ``f()`` is the system dynamics function.
        control (Control): holds the 'current' control data, computed at each step as:
...
class Simulator:
    ...
    def __init__(
        ...
        self._time: np.ndarray = np.zeros(0, dtype=np.float32)
        self._max_time: np.ndarray = np.zeros(0, dtype=np.float32)
        self._steps: np.ndarray = np.zeros(0, dtype=np.int32)
        self._max_steps: np.ndarray = np.zeros(0, dtype=np.int32)
        self._dt: np.ndarray = np.zeros(0, dtype=np.float32)
        ...
        self._simdata = SimulatorData(model=self._model)
    ...
    @property
    def simdata(self) -> SimulatorData:
        """
    ...
    def step(self):
        ...
        # Step the physics solver
        self._solver.step(
            state_in=self._simdata.state_p,
            state_out=self._simdata.state_n,
            control=self._simdata.control,
            contacts=self._contacts,
        )
